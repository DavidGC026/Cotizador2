...
foreach ($libros_seleccionados as $libro) {
    $libro = $libro['titulo'];
    $sql_producto = "SELECT titulo FROM libros WHERE titulo = ?";
    $stmt_producto = $conn->prepare($sql_producto);
    $stmt_producto->bind_param("s", $libro);
    $stmt_producto->execute();
    $result_producto = $stmt_producto->get_result();
    $row_producto = $result_producto->fetch_assoc();
    $nombre_producto = $row_producto['titulo'];

    $nombre_productos[] = $nombre_producto;
}

if (count($nombre_productos) === 1) {
    $mensaje = "Agradecemos de antemano la confianza depositada en el IMCYC para desarrollar la presente cotización respecto al libro $nombre_productos[0]).";
} else {
    $productos_texto = implode(', ', $nombre_productos);
    $mensaje = "Agradecemos de antemano la confianza depositada en el IMCYC para desarrollar la presente cotización respecto a los libros $productos_texto.";
}
...
$html .= '</tr>';
$html .= '<tr>';
$html .= '<th style="text-align: center; height: 10px;">Título</th>';
$html .= '<th style="text-align: center; height: 10px;">Autor</th>';
$html .= '<th style="text-align: center; height: 10px;">Precio</th>';
$html .= '<th style="text-align: center; height: 10px;">Cantidad</th>';
...
foreach ($libros_seleccionados as $fila) {
    $html .= '<tr>';
    $html .= '<td style="text-align: center;">'. htmlspecialchars($fila['titulo']) . '</td>';
    $html .= '<td style="text-align: center;">' . htmlspecialchars($fila['autor']) . '</td>';
...
$html .= '<table border="1">';
$html .= '<tr>';
$html .= '<td colspan="4" style="text-align: right; padding: 5px;">' . 'Subtotal: ' . '</td>';
$html .= '<td colspan="2" style="text-align: center; padding: 5px;">$' . number_format($subtotal, 2) . '</td>';
$html .= '</tr>';
$html .= '<tr>';
$html .= '<td colspan="4" style="text-align: right; padding: 5px;">' . 'Subtotal con Descuento: ' . '</td>';
$html .= '<td colspan="2" style="text-align: center; padding: 5px;">$' . number_format($subtotal_snd, 2) . '</td>';
$html .= '</tr>';
...
$grantotal = $subtotal_snd + $costoEnvio;
$html .= '<table border="1">';
$html .= '<tr>';
$html .= '<td colspan="4" style="text-align: right; padding: 5px;">' . 'Costo de Envio' . '</td>';
$html .= '<td colspan="2" style="text-align: center; padding: 5px;">$' . number_format($costoEnvio, 2) . '</td>';
$html .= '</tr>';
$html .= '<tr>';
$html .= '<td colspan="4" style="text-align: right; padding: 5px;">' . 'Gran Total' . '</td>';
$html .= '<td colspan="2" style="text-align: center; padding: 5px;">$' . number_format($grantotal, 2) . '</td>';
$html .= '</tr>';
$html .= '</table>';
