...
foreach ($libros_seleccionados as $curso) {
    $curso = $curso['nombre'];
    $sql_producto = "SELECT nombre FROM Webinars WHERE nombre = ?";
...
if (count($nombre_productos) === 1) {
    $mensaje = "Agradecemos de antemano la confianza depositada en el IMCYC para desarrollar la presente cotización respecto a la Certificacion $nombre_productos[0].";
} else {
    $productos_texto = implode(', ', $nombre_productos);
    $mensaje = "Agradecemos de antemano la confianza depositada en el IMCYC para desarrollar la presente cotización respecto a las Certificaciones $productos_texto.";
}
foreach ($libros_seleccionados as $curso) {
$curso = $curso['nombre'];
$pdf->SetFont('helvetica', '', 10);
$pdf->SetTextColor(0, 51, 153);
$pdf->Cell(0, 4, "WEBINAR / EXAMEN $curso", 0, 1, 'L');
$pdf->Ln();
}
...
$html .= '</tr>';
$html .= '<tr>';
$html .= '<th style="text-align: center; height: 10px;"></th>';
$html .= '<th style="text-align: center; height: 10px;">COSTO</th>';
$html .= '<th style="text-align: center; height: 10px;">IVA</th>';
$html .= '<th style="text-align: center; height: 10px;">PERSONAS</th>';
$html .= '<th style="text-align: center; height: 10px;">DESCUENTO</th>';
$html .= '<th style="text-align: center; height: 10px;">TOTAL</th>';
$html .= '</tr>';
    ...
    $subtotal += $importe;
    $ivap = $importe_descuento * 0.16;
    $totalp = $ivap + $importe_descuento;
    $html .= '<tr>';
    ...
    $html .= '<td style="text-align: center;"> $' . htmlspecialchars($ivap) . '</td>';
    ...
    $html .= '<td style="text-align: center;">$' . htmlspecialchars($totalp) . '</td>';
    $html .= '</tr>';

    $costo_examen = 1000;
    $importe12 = $cantidad * $costo_examen;
    $importe_descuento12 = $importe12 - ($importe12 * ($descuento/100));
    $ivap12 = $importe_descuento12 * 0.16;
    $totalp12 = $ivap12 + $importe_descuento12;
    $grantotal += $totalp + $totalp12;
    $subtotal_snd += $importe_descuento + $importe_descuento12;

    $html .= "<tr>";
    $html .= '<td style="text-align: center;">  Examen </td>';
    $html .= '<td style="text-align: center;">' .  htmlspecialchars($costo_examen) . '</td>';
    $html .= '<td style="text-align: center;"> $' . htmlspecialchars($ivap12) . '</td>';
    $html .= '<td style="text-align: center;">' . htmlspecialchars($fila['cantidad']) . '</td>';
    if ($fila['descuento'] == ""){
        $html .= '<td style="text-align: center;">' . ' ' . '</td>';    
    }else{
    ...
    $html .= '<td style="text-align: center;">$' .  htmlspecialchars($totalp12) . '</td>';
    $html .= "</tr>";
    $html .= '</table>';
    $html .= '<p></p>';
    ...
    $html .= '<table border="1">';
    $html .= "<tr>";
    $html .= '<td style="text-align: center;">  Gran Total </td>';
    $html .= '<td style="text-align: center;">' .  htmlspecialchars(($costo_examen + $precioUnidad)) . '</td>';
    $html .= '<td style="text-align: center;"> $' . htmlspecialchars(($ivap12 + $ivap)) . '</td>';
    $html .= '<td style="text-align: center;">' . htmlspecialchars($fila['cantidad']) . '</td>';
    if ($fila['descuento'] == ""){
        $html .= '<td style="text-align: center;">' . ' ' . '</td>';    
    }else{
    ...
    $html .= '<td style="text-align: center;">$' .  htmlspecialchars(($totalp12 + $totalp)) . '</td>';
    $html .= "</tr>";
    $html .= '</table>';
    ...
    $html .= '<table border="1">';
    $html .= '<tr>';
    $html .= '<td rowspan="4" style="text-align: center;">' . 'El WEBINAR INCLUYE' . '</td>';
    $html .= '<td colspan="2" style="text-align: center;">';
    $descripcion = $fila['descripcion'];
    $valores = explode(',', $descripcion);
...
$pdf->SetFont('helvetica', '', 10);
$pdf->Ln();
$pdf->MultiCell(0, 10, 'Fecha a definir en común acuerdo para la aplicación del examen en nuestro Instituto.', 0, 'F');
$pdf->MultiCell(0, 10, 'Una vez echo el pago se le envira una liga con acceso y contraseña para estudiar el 
WEBINAR, el cual tendrá una vigencia de 90 días. ', 0, 'F');
...
$filename = 'cotizacion_webinars_' . date('YmdHis') . '.pdf';
...
foreach ($libros_seleccionados as $fila) {
    ...
    $nombre = $fila['nombre'];
    $cantidad = $fila['cantidad'];
    $sql_ventas = "SELECT ventas FROM Webinars WHERE nombre = ?";
    ...
    if ($result_ventas->num_rows > 0) {
        ...
        $sql_actualizar_ventas = "UPDATE Webinars SET ventas = ? WHERE nombre = ?";
