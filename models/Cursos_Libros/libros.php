...
<body>
    ...
    $conn_cliente = new mysqli($servername, $username, $password, $dbname);
    ...
    $sql_general = "SELECT * FROM libros";
    ...
    ?>
    <div class="container">
        <h1>Generando Cotización para <?php echo $usuariopr?></h1>
            ...
            <tbody>
                ...
                foreach ($data as $row) {
                    ...
                    if ($row['precio'] == 0) {
                        ...
                        echo "<td><input type='number' name='precioVenta[$id]' class='precioVenta' value='{$row['precio']}'></td>";
                    }
                    echo "<td><input type='number' class='cantidad' min='1' max='10'></td>";
                    echo "<td><input type='number' class='descuento' min='1' max='100'></td>";
                    echo "<td></td>";
                    echo "<td></td>";
                    echo "<td><input type='checkbox' class='seleccion' data-precio='{$row['precio']}'></td>";
                    echo "</tr>";
                }
                ?>
            </tbody>
        ...
        <table>
            <tr>
                <td>
                    <button id="continuar" class="btn btn-primary">Continuar</button> <!-- Corregir duplicación de atributo style -->
                    <button type="submit" name="regresar" id="regresar" style="display: none; background-color: #001f3f;" class="btn btn-primary">Regresar</button>
                </td>
                <td>
                <form method="post" action="generar_factura.php" id="pdfForm" target="_blank">
                    <table border="0px">
                        <tr>
                            <td><label id="costoenviotxt" style="display: none";>Costo de Envio:</label></td>
                            <td><input type="text" id="costoenvio" name="costoenvio" style="display: none";></td>
                            <td>
        ...
        <div class="total" id="totalContainer" style="display: none;">
            Total: <span id="precio_total">0</span><br>
        </div>
    </div>
    ...
    <script>
        ...
        function mostrarBotones() {
            ...
            document.getElementById("costoenviotxt").style.display = "block";
        ...
        function mostrarConsultarPorId() {
            ...
            document.getElementById("costoenviotxt").style.display = "none";
        ...
        function mostrarResultado(data) {
            ...
            table.innerHTML = '<tr><th>ID</th><th>General</th><th>Titulo</th><th>Autor</th><th>Precio</th><th>Cantidad</th><th>Descuento</th><th>Total</th><th>Total Final</th><th>Seleccionar</th></tr>';
            ...
            data.forEach(item => {
                ...
                var campo4Cell = row.insertCell(4);
                var cantidadCell = row.insertCell(5);
                var descuentoCell = row.insertCell(6);
                var totalCell = row.insertCell(7);
                var totalDescCell = row.insertCell(8);
                var checkboxCell = row.insertCell(9);

                idCell.textContent = item.id;
                campo1Cell.textContent = item.general;
                campo2Cell.textContent = item.titulo;
                campo3Cell.textContent = item.autor;
                campo4Cell.innerHTML = `<input type='number' class='precioVenta'>`;
        ...
        function actualizarTabla() {
            ...
            for (var i = 0; i < checkboxes.length; i++) {
                if (checkboxes[i].checked) {
                    ...
                    var totalCell = fila.cells[7];
                    var totalDescCell = fila.cells[8];
        ...
        function calcularPrecioTotal() {
            ...
            for (var i = 0; i < checkboxes.length; i++) {
                if (checkboxes[i].checked) {
                    subtotal += parseFloat(checkboxes[i].parentNode.parentNode.cells[8].textContent);
                }
            }

            // Cambié precioTotalFinal por subtotal
            document.getElementById('precio_total').textContent = subtotal.toFixed(2);
        ...
        function obtenerDatosTabla() {
            ...
            for (var i = 1; i < table.rows.length; i++) {
                ...
                var general = row.cells[1].textContent;
                var titulo = row.cells[2].textContent;
                var autor = row.cells[3].textContent;
                var precio = row.cells[4].querySelector('.precioVenta').value;
                var cantidad = row.cells[5].querySelector('.cantidad').value;
                var descuento = row.cells[6].querySelector('.descuento').value;
                var total = row.cells[7].textContent;
                var totalDesc = row.cells[8].textContent;
                var seleccionado = row.cells[9].querySelector('.seleccion').checked;
